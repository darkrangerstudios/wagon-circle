...
// A = Wagon Wheel: one long-lived `claude -p` stream-json session, each turn sends only the new message.
...
(async () => {
  const a = await runA();
  const b = runB();
  for (const [name, rows] of [['A  Wagon Wheel (persistent session, deltas)', a], ['B  naive (new process, full transcript)', b]]) {
    console.log(`\n${name}`);
