...
function describeFile(fp, mtime) {
  ...
  return { id: path.basename(fp, '.jsonl'), path: fp, cwd, mtime, title: t ? (t.customTitle || t.aiTitle) : null, preview: textOf(first).slice(0, 100), mode: typeof first.permissionMode === 'string' ? first.permissionMode : null, entrypoint: typeof first.entrypoint === 'string' ? first.entrypoint : null };
}
